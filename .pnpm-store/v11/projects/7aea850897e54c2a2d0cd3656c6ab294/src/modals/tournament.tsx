...
// ---------------------------------------------------------------------------
// Shared tournament info fields (name, stake, provider tournament)
// ---------------------------------------------------------------------------
...
function TournamentInfoFields({
  ...
  const { data: providerTournaments, isLoading: providersLoading } = useListProviderTournamentsQuery()
  return (
    <>
      <div>
      ...
      <div>
        <FieldLabel>Provider tournament <i>(recommended)</i></FieldLabel>
        {providersLoading ? (
          <div className={`${fieldClass} flex items-center gap-2 text-gray-400 dark:text-gray-500`}>
            <Loader2 className="animate-spin h-4 w-4 flex-shrink-0" />
...
export function CreateTournamentModal({ onClose }: { onClose: () => void }) {
  ...
  async function handleCreate() {
    ...
    setError(null)
    try {
      const selectedProvider = providerSelection(externalId)
